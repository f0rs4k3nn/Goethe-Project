...
void ComputeShadowTextureCoords(float3 rayDir, float t0, float t1) {
    shadowTextureStart = mul(_ShadowMatrix, float4(_WorldSpaceCameraPos + rayDir * t0, 1.0));
    shadowTextureEnd = mul(_ShadowMatrix, float4(_WorldSpaceCameraPos + rayDir * t1, 1.0));
}

half SampleShadowMap(float4 shadowCoords) {
