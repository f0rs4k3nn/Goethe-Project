...
#define CONE_COS_SQR _ExtraGeoData.w
...
float ConeIntersection(float3 origin, float3 viewDir) {
    ...
    //caps
    if( m1<0.0 ) {
        if( dot2(oa*m2-viewDir*m1)< CONE_TIP_RADIUS*CONE_TIP_RADIUS*m2*m2 ) // delayed division
            return -m1/m2;
    }
    else if( m9>0.0 ) {
...
float SphereAttenuation(float3 wpos) {
    ...
    return distSqr * _ExtraGeoData.y + _ExtraGeoData.z;
}
