Shader "VolumetricLights/VolumetricLightURP"
{
	Properties
	{
		[HideInInspector] _MainTex("Noise Texture", Any) = "white" {}
		...
		SubShader
		{
			...
			Pass
			{
                ...
                #pragma multi_compile_local VL_SPOT VL_POINT VL_AREA_RECT VL_AREA_DISC
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					float t0 = ComputeIntersection(_WorldSpaceCameraPos, rayDir);
                    float t1 = distance(_WorldSpaceCameraPos, i.wpos);

                    #if VL_CUSTOM_BOUNDS && !VL_AREA_RECT
                        float b0, b1;
                        BoundsIntersection(_WorldSpaceCameraPos, rayDir, b0, b1);
                        t0 = max(t0, b0);
                        t1 = min(t1, b1);
                    #endif

					ClampRayDepth(i.scrPos, t1);
                    if (t0>=t1) return 0;

					SetJitter(i.scrPos);
                    t0 += jitter * _JitterStrength;

                    #if VL_SHADOWS
                        ComputeShadowTextureCoords(rayDir, t0, t1);
                    #endif

					half4 color = Raymarch(rayDir, i.scrPos, t0, t1);
					return color;
				}
				ENDHLSL
			}
